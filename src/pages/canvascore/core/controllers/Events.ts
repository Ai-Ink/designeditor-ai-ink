...
class Events extends Base {
  ...
  private initialize() {
    this.canvas.view.element.tabIndex = 1;
    this.canvas.view.element.style.outline = 'none';
    // @ts-ignore
    this.canvas.on({
    ...
    this.canvas.view.element.addEventListener(
      'keydown',
  ...
  public destroy() {
    ...
    this.canvas.view.element.removeEventListener(
      'keydown',
      this.onKeyDown.bind(this),
    );
  }
  ...
  handleZoom = (event: fabric.IEvent<any>) => {
    ...
    this.editor.zoom.zoomToPoint(
      new fabric.Point(
        this.canvas.view.viewSize.width / 2,
        this.canvas.view.viewSize.height / 2,
      ),
      zoomRatio,
    );
  ...
  onBackgroundSelected = () => {
    // const objects = this.canvas.getItems({});
  ...
  handleSelection = (target: fabric.IEvent) => {
    if (target) {
      ...
      if (
        ...
        const filteredObjects = (
          initialSelection._objects as fabric.Object[]
        ).filter((object) => {
    ...
    // No explicit rendering required.
    this.canvas.view.draw();
  };
